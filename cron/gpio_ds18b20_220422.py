    ...
    try:
        ...
        for i in range(0, len(temperature)):
            ...
            if mode == 0 or (cur.rowcount == 0 or (cur.rowcount > 0 and ((round(payload, 1) != round(last_message_payload, 1)) or tdelta > sensor_timeout))):
                ...
                print("mode - ",mode,"   rowcount - ",cur.rowcount)
                print("tdelta - ",tdelta,"   sensor_timeout - ",sensor_timeout)
                print("payload - ",round(payload, 1),"   last_payload - ",round(last_message_payload, 1))
                cur.execute(
